...
class GitlabIssue < ApplicationRecord
  ...
  def self.find_by_gitlab_identifiers(id: nil, url: nil, initialize: false)
    ...
    # project, issues from different repositories routinely share an iid, so
    # matching on +gitlab_id+ alone collapses them into a single record. The web
    # URL is globally unique and stable, so prefer it whenever present and only
    # fall back to +gitlab_id+ when no URL is available.
    found =
      if url.present?
