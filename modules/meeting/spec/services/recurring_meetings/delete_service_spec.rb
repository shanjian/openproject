...
RSpec.describe RecurringMeetings::DeleteService do
  ...
  subject { described_class.new(user: current_user, model: series).call }

  before do
    series.template.update_column(:notify, false)
    series.template.participants.delete_all
    series.template.participants << MeetingParticipant.new(user: participant_user, invited: true)
    ActionMailer::Base.deliveries.clear
  end

  it "sends the series cancellation mail even when the series is muted" do
  ...
  it "clears a pending batched series update job" do
    ...
    expect(RecurringMeetings::SendUpdatedNotificationJob).to have_received(:delete_jobs).with(series)
  end

  context "with a draft series" do
    before do
      series.template.update_column(:state, Meeting.states[:draft])
    end
