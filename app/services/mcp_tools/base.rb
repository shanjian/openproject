...
module McpTools
  class Base
    ...
    def format_response(result)
      # The mcp gem performs strict type checks on the structured content, so the representers our tools
      # return have to be converted into plain Ruby hashes before the gem sees them. Output filters
      # operate on that converted structure, so this single round-trip serves both. Upstream repeats it
      # in #format_structured_content, where it can only ever be a no-op on an already-converted hash.
