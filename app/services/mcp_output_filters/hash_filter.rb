...
module McpOutputFilters
  ...
  class HashFilter
    def filter(hash_or_array)
      ...
      # Filtering happens in place, but McpTools::Base#format_response threads the return value
      # through its chain of filters. Returning the case expression instead, as upstream does, yields
      # nil whenever #on_hash stops the descent at the top level -- so a tool whose result is a HAL
      # document with top-level _links would have its entire response body replaced by nil. All three
      # of our ResourceProxyTool subclasses return exactly that shape.
      hash_or_array
    end
