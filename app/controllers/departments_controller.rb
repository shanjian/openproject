...
class DepartmentsController < ApplicationController
  ...
  def add_user # rubocop:disable Metrics/AbcSize
    ...
    if result.success?
      ...
      redirect_to department_path(@group), status: :see_other
    elsif result.result.is_a?(Group)
      respond_with_dialog(
        Departments::MoveUserDialogComponent.new(
  ...
  def change_parent_dialog
    ...
    respond_with_dialog(
      Departments::ChangeParentDialogComponent.new(department: @group, departments:)
    )
  end
  ...
  def update
    ...
    if service_call.success?
      ...
      redirect_to edit_department_path(@group), status: :see_other
    else
  ...
  def destroy
    ...
    redirect_to redirect_target ? department_path(redirect_target) : departments_path, status: :see_other
  end
  ...
  def destroy_membership
    ...
    redirect_to edit_department_path(@group, tab: redirected_to_tab(member)), status: :see_other
  end
  ...
  def respond_parent_changed(service_call)
    if service_call.success?
      flash[:notice] = I18n.t(:notice_successful_update)
      redirect_to department_path(service_call.result.parent || service_call.result), status: :see_other
    else
      flash[:error] = service_call.errors.full_messages.join("\n")
      redirect_to department_path(@group), status: :see_other
    end
  ...
  def respond_department_created(service_call)
    if service_call.success?
      ...
      redirect_to department_path(redirect_target_for(service_call.result)), status: :see_other
    else
      flash[:error] = service_call.errors.full_messages.join("\n")
      redirect_back_or_to(departments_path)
    end
  end
  ...
  def respond_membership_altered(service_call)
    ...
    redirect_to edit_department_path(@group, tab: redirected_to_tab(service_call.result))
  end
