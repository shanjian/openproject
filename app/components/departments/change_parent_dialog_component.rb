...
module Departments
  class ChangeParentDialogComponent < ApplicationComponent
    ...
    def form_arguments
      {
        id: FORM_ID,
        url: change_parent_department_path(@department),
        method: :post
      }
    end
    ...
    def disabled_ids
      # Disallow the department itself, its current parent and descendants (circular).
      # (No LDAP sync module is installed in this fork, so no department is ever LDAP-managed.)
