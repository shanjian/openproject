  ...
  class DepartmentRowComponent < ApplicationComponent
    ...
    def call
      flex_layout(align_items: :center, justify_content: :space_between) do |row|
        row.with_column do
          render(Primer::Beta::Link.new(href: department_path(@department))) { @department.name }
        end
    ...
    # No LDAP sync module is installed in this fork, so a department is never LDAP-managed.
    def managed?
      false
    end
    ...
    def add_sub_department_item(menu)
      menu.with_item(
        ...
        tag: :a,
        href: new_department_departments_path(parent_id: @department.id),
        content_arguments: { data: { turbo_frame: Departments::HierarchyLayoutComponent.wrapper_key } }
    ...
    def add_user_item(menu)
      menu.with_item(
        ...
        tag: :a,
        href: new_user_department_path(@department),
        content_arguments: { data: { turbo_frame: Departments::HierarchyLayoutComponent.wrapper_key } }
    ...
    def change_parent_item(menu)
      menu.with_item(
        ...
        tag: :a,
        href: change_parent_department_path(@department),
    ...
    def delete_item(menu)
      menu.with_item(
        ...
        scheme: :danger,
        tag: :a,
        href: department_path(@department),
        content_arguments: {
          data: {
