...
module Pages
  class Departments < ::Pages::Page
    ...
    # This fork has no Turbo-specific Capybara wait helpers (unlike upstream, which
    # ships its own). Without synchronizing here, a subsequent action in the same
    # example (e.g. another click) can fire while the previous Turbo Frame/Stream
    # fetch is still in flight, producing a second, racy request against a DOM
    # Turbo is mid-way through replacing. We wait for the request the block
    # triggers to actually start, then for the network to go idle again, which
    # covers both cases (Cuprite/Ferrum's network traffic log via CDP).
    def wait_for_turbo_frame(&)
      network = page.driver.browser.network
      before = network.traffic.size

      yield

      Timeout.timeout(Capybara.default_max_wait_time) do
        sleep 0.05 until network.traffic.size > before
      end
      network.wait_for_idle(duration: 0.1, timeout: Capybara.default_max_wait_time)
    end
    alias_method :wait_for_turbo_stream, :wait_for_turbo_frame
    alias_method :wait_for_turbo, :wait_for_turbo_frame

    private

    def organization_name_selector
      "##{::Departments::OrganizationNameComponent.wrapper_key}"
    end

    def detail_component_selector
      "##{::Departments::DetailComponent.wrapper_key}"
    end
