...
RSpec.describe McpTools::SearchCustomFields, with_flag: { mcp_server: true } do
  ...
  # Attached to a type in a project, because this fork keeps upstream's older CustomField.visible
  # semantics: visibility runs through on_visible_type_and_project rather than short-circuiting for
  # admins. Bare custom fields on an instance with no projects at all are invisible to everyone,
  # which is not a situation a real instance is ever in.
  let(:type) { create(:type) }
  let!(:project) { create(:project, types: [type]) }
  let!(:custom_field_one) { create(:wp_custom_field, name: "One custom field", types: [type], projects: [project]) }
  let!(:custom_field_two) { create(:wp_custom_field, name: "Another field", types: [type], projects: [project]) }
