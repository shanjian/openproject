...
RSpec.describe McpTools::SearchCustomFieldItems, with_flag: { mcp_server: true } do
  ...
  # Attached to a type in a project: this fork keeps upstream's older CustomField.visible semantics,
  # which run through on_visible_type_and_project rather than short-circuiting for admins.
  let(:type) { create(:type) }
  let!(:project) { create(:project, types: [type]) }

  let!(:custom_field) do
    create(:wp_custom_field, :hierarchy, types: [type], projects: [project]).tap do |cf|
  ...
  context "without an Enterprise token", with_ee: %i[custom_field_hierarchies] do
