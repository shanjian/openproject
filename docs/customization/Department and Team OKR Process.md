...
There is no separate "level" to assign by hand on each Objective or Key Result - a department's position in the Organizational Unit tree (Company / department / team) is what determines which of these three views it belongs in, so an individual item can't drift out of sync the way a manually-set field could.

The views themselves are generated by `scripts/create_okr_saved_views.rb`, which captures the current department/team ids at the time it runs. They do **not** update automatically: rerun the script whenever a department or team is added, removed, or reparented, and whenever the quarter rolls over (see End-of-Quarter Process below) - it is safe to rerun at any time. A rerun also removes any Department/Team Objectives view for a department or team that no longer exists, so a rename or removal doesn't leave a stale view behind.
...
10. Rerun `scripts/create_okr_saved_views.rb` with `OKR_VERSION` set to the new quarter, so the Executive Review views point at the new Version instead of the closed one
